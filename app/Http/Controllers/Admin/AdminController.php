...
public function unresolve_status(Request $r,$id)
{
  ...
  */
  ...
  public function due_date_resolve() {
     $data=DB::table('mct_beneficiary')->where('date_status', '=', 1)->get();//date_status=0 implies that delivery date is resolved.
     $x=0;
     foreach($data as $i)
     {
        $cc_id=DB::table('mct_due_list')->where('fk_b_id',$i->b_id)->first()->fk_cc_id;
        if($cc_id != NULL){
        $cc_user_id=DB::table('mct_call_champions')->where('cc_id',$cc_id)->first()->fk_user_id;}
        $field_worker_id=DB::table('mct_field_workers')->where('f_id',$i->fk_f_id)->first()->fk_user_id;
        $newdata[$x]['b_id']=$i->b_id;
        $newdata[$x]['b_name']=$i->v_name;
        $newdata[$x]['field_worker_name']=DB::table('mct_user')->where('user_id',$field_worker_id)->first()->v_name;
        $newdata[$x]['call_champion_name']=DB::table('mct_user')->where('user_id',$cc_user_id)->first()->v_name;
        $newdata[$x]['rd_date']=$i->reported_delivery_date;
        $newdata[$x]['due_date']=$i->dt_due_date;
        $x++;          
     }
     return view('admin/due_date_resolve',compact('newdata'));
    }

  public function due_date_accept(Request $r,$id, $ddate) {
    //DB::table('mct_callchampion_report')->where('report_id',$id)->update(['status'=>1]);
    $obj = new BeneficiaryController;
    $obj->update_beficiary_delivery_date($id, $ddate);
    DB::table('mct_beneficiary')->where('b_id',$id)->update(['date_status'=>0]);
    return back();  
  }
  public function due_date_reject(Request $r,$id) {
    DB::table('mct_beneficiary')->where('b_id',$id)->update(['date_status'=>0]);
    return back();  
  }
