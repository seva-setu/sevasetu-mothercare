              	...
              	?>
			        <tr>
                      <td><input type="checkbox" name="chkCheckedBox[]" id="chkCheckedBox" value="<?php echo $value->bi_id; ?>_<?php echo $value->bi_user_login_id; ?>" /></td>
                      	<td><a href="<?php echo Config::get('app.url').'admin/callchampions/view/'.Hashids::encode($value->bi_id) ?>"><?php echo $value->v_unique_code;?></a></td>
                        <td><a href="<?php echo Config::get('app.url').'admin/callchampions/view/'.Hashids::encode($value->bi_id) ?>"><?php echo $value->v_name;?></a></td>
						...
						<td><?php echo $value->v_phone_number;?></td>
					  	...
					  	<td style="width: 230px" >
                      		...
                      		<a class="btn btn-info" href="<?php echo Config::get('app.url').'admin/callchampions/edit/'.Hashids::encode($value->bi_id) ?>"><i class="icon-edit icon-white"></i><?php echo trans('routes.edit'); ?></a>
							...
							if($value->e_status=="Active"){?>
                      			<a class="btn btn-info" href="<?php echo Config::get('app.url').'admin/callchampions/delete/'.Hashids::encode($value->bi_id)."/".Hashids::encode($value->bi_user_login_id)."/".Hashids::encode(0); ?>"></i><?php echo trans('routes.inactive'); ?></a>					  	
                      		<?php }elseif($value->e_status=="Inactive"){?>
                      			<a class="btn btn-info" href="<?php echo Config::get('app.url').'admin/callchampions/delete/'.Hashids::encode($value->bi_id)."/".Hashids::encode($value->bi_user_login_id)."/".Hashids::encode(1); ?>"></i><?php echo trans('routes.active'); ?></a>					  	
                      		<?php }}?>
                      		<?php if(isset($value->bi_calls_champion_id)){?>
                      		<a class="btn btn-info" href="<?php echo Config::get('app.url').'admin/assignbeneficiary/'.Hashids::encode($value->bi_id) ?>"><?php echo trans('routes.assignben'); ?></a>
							<?php }?>
						</td>
                    </tr>                	
                <?php } ?>
